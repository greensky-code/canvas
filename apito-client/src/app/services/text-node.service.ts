...
export class TextNodeService {
  ...
  textNode(stage: Konva.Stage, layer: Konva.Layer, color) {
    let socket = this.socket
    console.log(socket, this.socket);
    
    let done = false;
    let textUpdate = this.textUpdated
    ...
    // layer.fire('fontSizeChange')
    textNode.on('fillChange', () => {
      textNode.fill(this.color)
      layer.add(textNode);
      layer.draw()
      
      
    })
    textNode.on('fontSizeChange', () => {
      layer.add(textNode);
      layer.draw()
    ...
    return textUpdate.asObservable();
  }
}
